  ...
  return (
    <group ref={groupRef}>
      <mesh position={[2.6, 0.4, 0]}>
        ...
        <meshStandardMaterial color="#d946ef" roughness={0.3} metalness={0.2} />
      </mesh>
      <mesh position={[-2.3, -0.6, 0.8]}>
        <sphereGeometry args={[0.14, 24, 24]} />
        <meshStandardMaterial color="#ddd6fe" roughness={0.4} metalness={0.1} />
      </mesh>
...
export function Hero3D() {
  return (
    ...
    >
      ...
      <directionalLight position={[-3, -2, -3]} intensity={1.8} color="#d946ef" />
      <Environment preset="city" environmentIntensity={0.9} />
      <Sparkles count={70} scale={5.5} size={2.5} speed={0.3} color="#ddd6fe" opacity={0.9} />
      <DistortedBlob />
      <OrbitingSatellites />
    </Canvas>
  );
